...
import { getAllUsersApi, updateUserDetailsApi } from '../../../../features/all_users_admin/allUsersAPI';
...
const UpdateDetails: React.FC<UserProps> = ({ user, onClose }) => {
    ...
    return (
        <div className='update-details'> {/* This div is not styled in the original code */}
            <h1>עריכת פרטים</h1>
            <div>
                <h3>{user.email}</h3>
                <label htmlFor="first_name">שם פרטי:</label>
                <input type="text" id="first_name" placeholder={user.first_name} onChange={handleChange} name="first_name" />
                <label htmlFor="last_name">שם משפחה:</label>
                <input type="text" id="last_name" placeholder={user.last_name} onChange={handleChange} name="last_name" />
                {/* gender */}
                <label htmlFor="gender">מגדר:</label>
                <select value={userDetails.gender} onChange={handleChange} name="gender" id="gender">
                            <option value="">בחר מגדר</option>
                            <option value="זכר">זכר</option>
                            <option value="נקבה">נקבה</option>
                </select>
                <label htmlFor="phone_number">מספר טלפון:</label>
                <input type="text" id="phone_number" placeholder={user.phone_number} onChange={handleChange} name="phone_number" />
                {phoneValidation && <div className="error-message">{phoneValidation}</div>}
                <label htmlFor="birth_date">תאריך לידה:</label>
                <input type="date" id="birth_date" value={userDetails.birth_date} onChange={handleChange} name="birth_date" />
