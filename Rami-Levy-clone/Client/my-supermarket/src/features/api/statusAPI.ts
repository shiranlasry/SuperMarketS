...
export const addNewStatusApi = async (department_status_id: number, department_status_name: string) => {
    try {
        const response = await axios.post("/api/status/add-new-status", { department_status_id, department_status_name });
        const { ok, results } = response.data;
        if (!ok) {
...
export const updateStatusApi = async (status_id: number, department_status_id: number, department_status_name: string) => {
    try {
        const response = await axios.patch("/api/status/update-status", { status_id, department_status_id, department_status_name });
        const { ok, results } = response.data;
        if (!ok) {
...
export const deleteStatusApi = async (status_id: number) => {
    try {
        const response = await axios.delete(`/api/status/delete-status/${status_id}`);
        const { ok, results } = response.data;
        if (!ok) {
