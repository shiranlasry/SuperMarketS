...
export const addNewUserAddress = async (req: Request, res: Response) => {
    try { 
      const { user_id,city_id,street_id,floor,apartment,zip_code,phone_number ,is_default,address_name,house_number} = req.body;
      
      if (!user_id || !city_id || !street_id || !floor || !apartment || !zip_code || !phone_number  || !address_name || !house_number || is_default === undefined) {
      ...
      const query = "INSERT INTO addresses (user_id,is_default,city_id,street_id,floor,apartment,zip_code,phone_number,address_name,house_number) VALUES (?,?,?,?,?,?,?,?,?,?)";
      connection.query(query, [user_id,is_default,city_id,street_id,floor,apartment,zip_code,phone_number,address_name,house_number], (err, result) => {
        if (err) {
          console.error(err)
          return res.status(500).send({ ok: false, error: err })
        }
