...
export const getUserActiveCart = async (req: express.Request, res: express.Response) => {
    try {
        ...
        const checkQuery = "SELECT * FROM rami_levy_db.carts WHERE user_id = ? AND status_id = 1;";
        connection.query(checkQuery, [user_id], (err, results, fields) => {
            try {
                ...
                if (results.length > 0) {
                    ...
                    const insertQuery = "INSERT INTO rami_levy_db.carts (user_id, status_id) VALUES (?, ?);";
                    connection.query(insertQuery, [user_id, 1], (err, results, fields) => {
                        try {
                            if (err) throw err;
                            // Select the newly inserted cart
                            const selectQuery = "SELECT * FROM rami_levy_db.carts WHERE user_id = ? AND status_id = 1;";
                            connection.query(selectQuery, [user_id], (err, results, fields) => {
                                try {
                                    if (err) throw err;
                                    res.send({ ok: true, results });
                                } catch (error) {
